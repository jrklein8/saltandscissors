...
-- 1) Adds the columns the updated form uses (safe to re-run)
-- 2) Imports Rebecca's first two real events into her account
--    (guarded: re-running will NOT create duplicates)
...
alter table events add column if not exists price_lines jsonb not null default '[]'::jsonb;

-- 2) Import (owner = Rebecca's user id from Authentication -> Users)

-- Gracyn Janning — Stevie's 4th, Oct 3
insert into events (
  ...
  location, theme, notes, price_lines, price_quoted, price_agreed, deposit, deposit_paid, paid_in_full
)
select
  ...
  '[{"label":"Sensory Scenes · Playdough (includes 15)","amount":350},{"label":"5 extra guests × $10","amount":50}]'::jsonb,
  350, 400, 100, true, false
where not exists (
  select 1 from events where user_id = 'caa71c62-2cbc-4f4b-98e8-7a8a13ecd2a4'
    and client_name = 'Gracyn Janning' and event_date = '2026-10-03'
);

-- Brittany Schram — Skylar's 8th, Nov 7
insert into events (
  user_id, status, client_name, client_contact, source, occasion, honoree,
  experience, experience_detail, guest_count, event_date, event_time, event_end,
  location, theme, notes, price_lines, price_quoted, price_agreed, deposit, deposit_paid, paid_in_full
...
select
  ...
  '[{"label":"Coastal Creamery · Cloud slime (slime only, includes 15)","amount":350}]'::jsonb,
  350, 350, 150, true, false
where not exists (
  select 1 from events where user_id = 'caa71c62-2cbc-4f4b-98e8-7a8a13ecd2a4'
    and client_name = 'Brittany Schram' and event_date = '2026-11-07'
);

-- Check: both events with their balances ($300 and $200)
